...
export const VideoComponent = memo(function VideoComponent({ videos, editedVideo, reelVideo, productId, productTitle }) {
    ...
    const VideoCard = ({ video, index }) => {
        const downloadVideo = () => {
            ...
            link.download = `${video.title.toLowerCase().replace(/\s+/g, '_')}_${productId || 'artisan'}.mp4`;
            link.click();
        };
        ...
        const shareVideo = () => {
            if (navigator.share) {
                navigator.share({
                    title: `${video.title} - ${productTitle || 'Artisan Product'}`,
                    text: `Check out this ${video.title.toLowerCase()} for ${productTitle || 'artisan product'}!`,
                });
            }
        };
    ...
    if (!videoData.processedVideos || videoData.processedVideos.length === 0) {
        return (
